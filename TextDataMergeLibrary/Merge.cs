...
namespace TextDataMergeLibrary
{
        ...
        TextFunctionBase
    {
        ...
        /// <returns></returns>
        public Boolean Equals(Merge other)
        {
        ...
        void InputFiles_ListChanged(Object sender, ListChangedEventArgs e)
        {
            try
            {
                this.OnPropertyChanged(String.Format("Merge.InputFiles[{0}].{1}", e.NewIndex, (e.PropertyDescriptor == null ? String.Empty : e.PropertyDescriptor.Name)));
            }
            catch (Exception ex)
            {
                Log.Write(ex, MethodBase.GetCurrentMethod(), EventLogEntryType.Error);
        ...
        void OutputFiles_ListChanged(Object sender, ListChangedEventArgs e)
        {
            try
            {
                this.OnPropertyChanged(String.Format("Merge.OutputFiles[{0}].{1}", e.NewIndex, (e.PropertyDescriptor == null ? String.Empty : e.PropertyDescriptor.Name)));
            }
            catch (Exception ex)
            {
                Log.Write(ex, MethodBase.GetCurrentMethod(), EventLogEntryType.Error);
        ...
        public override Boolean Valid()
        {
            ...
            try
            {
                ...
                {
                    returnValue = false;
                    ErrorMessage = String.Format("InputFiles must contain at least one file.");
                }
        ...
        public override Boolean AddInputFile(String filePath)
        {
            ...
            try
            {
                returnValue = AddFile<DataFile>(filePath, this.InputFiles, new DataFile(filePath), false);
            }
            catch (Exception ex)
            {
                Log.Write(ex, MethodBase.GetCurrentMethod(), EventLogEntryType.Error);
